...
资源库里还有技能勾选（junction 分发）。建议一并删除：全局 `~/.claude/skills` 所有
agent 天生可见，个别 agent 专属技能手动放它工作目录 `.claude/skills/`。待拍板。
...
- **⏹ 打断 = Esc**：杀进程树、游标回退（消息不丢）、agent 进 `held` 状态——
  **不再自动重唤醒**。只有打断**之后**来的新消息才重新唤醒它，届时老积压 + 新消息
  合并送达，唤醒词带"你被打断了"标注（借 --resume 记忆接着干，不从头来）；
- 两个旧按钮**合并为一个 ⏹**，出现在：私聊头部（该 agent working/waiting 时）+
  详情悬浮卡 + 群聊成员卡——不用再去 agent 列表里翻；
- "Esc 后改口重说" = ⏹ 打断 + （树上线后）回撤到上一节点再发新消息，两个功能天然咬合。

### 归档 → 分组

归档和"不给它发消息"没有区别（agent 本来就只在被消息触发时才唤醒）。删除 archived
状态，`agents` 表加 `grp TEXT DEFAULT ''`：agent 列表按组折叠显示，组名随起随用。
迁移：现有 archived agent → 组"归档"、status 恢复 active。paused（连续失败自动暂停）保留。
...
- **节点粒度 = 一次唤醒**（一轮完整交互），不是一条消息——fork 只能发生在轮边界，
  且消息级节点会碎到没法用；
...
- /compact 也建节点（kind=compact，fork 后压缩），压缩前状态照样可回撤；
- 切分支 0 token；成本账见 §0。
...
1. **技能库**是否随资源库一起删（改用全局 ~/.claude/skills）？
2. 树节点粒度 = **每次唤醒一个节点**（非每条消息），可接受？
3. 归档 agent 迁移为"归档"分组、状态恢复 active，可接受？
4. held（被打断）状态的解除条件：**任何**新消息（含群聊里别人 @ 它）都唤醒，
   还是只有你本人的消息才唤醒？（建议前者，唤醒词会带打断标注）
