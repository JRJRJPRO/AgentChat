...
最省事：**双击 `AgentChat.vbs`**（可右键→发送到→桌面快捷方式）——后台起服务器并用 Edge
应用模式打开独立窗口。关闭：界面右上角 **⏻** 或运行 `stop.ps1`。
...
浏览器打开 **http://127.0.0.1:8787**。界面右上角可切换 🌗 深/浅色主题 和 文/A 中英文（都会记住）。
...
Agent 页顶部显示近 5 小时 AgentChat 自身的消耗（唤醒次数、输入/输出/缓存 token、按 API
价折算的金额——订阅用户不实际扣这笔钱，仅用于衡量额度消耗速度）。注意这统计不了你在
VSCode/终端里手动使用 Claude Code 的部分，官方 5 小时窗口余量目前没有本地接口可查。
